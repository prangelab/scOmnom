...
Sample enrichment writes six TSV tables and a run manifest to `tables/enrichment_sample_<round>_roundN/`, figures to `figures/<format>/enrichment_sample_<round>_roundN/`, logs to `logs/enrichment.sample.log`, and an `adata.enrichment_sample_<round>.zarr.tar.zst` dataset. Optional H5AD output uses the same stem. Each stored analysis has its own round-native identifier. Figure regeneration creates separate `<analysis_id>_regeneration_roundN` figure folders and manifests under `figures/regeneration/`; see the [sample command reference](markers-and-de/enrichment.md#sample-enrichment).
