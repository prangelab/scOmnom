...
* a machine-readable embedding-selection decision table
...
Integration metric and audit tables are written under `integration_metrics/`. Standard multi-batch benchmarking writes `integration_metrics_raw*.tsv`, `integration_metrics_scaled*.tsv`, and `integration_selection_decision*.tsv`. The decision table records scaled component scores, baseline deltas, eligibility, tier assignment, selection status, policy, and numerical tolerance. Single-batch runs write `integration_single_batch_selection*.tsv`; runs with no candidate beyond `Unintegrated` additionally write `integration_no_candidate_selection*.tsv`.
