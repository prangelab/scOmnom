...
def main():
    ...
    # We disable CellTypist to avoid model loading
    cfg = ClusterAnnotateConfig(
        input_path=None,          # we will not use file loading
        output_path=outdir / "synthetic.clustered.annotated.h5ad",
        embedding_key="X_umap",
        batch_key="batch",        # synthetic batch key
        label_key="leiden",

        # Allow biological guidance to show up
        ...
        # Disable CellTypist model loading
    ...
    # Directly call run_clustering() with in-memory AnnData
    # Save temporarily then load through pipeline
