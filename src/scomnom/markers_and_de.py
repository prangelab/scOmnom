...
def run_markers_and_de(cfg) -> ad.AnnData:
    ...
    Pseudobulk DE is automatically DISABLED if the dataset
    does not have sufficient sample replication.
    """
    ...
    if sample_key is None:
        raise RuntimeError(
            "markers_and_de: sample_key/batch_key missing "
            "(and adata.uns['batch_key'] not set)."
        )

    LOGGER.info("markers_and_de: groupby=%r, sample_key=%r", groupby, sample_key)

    # ------------------------------------------------------------------
    # GLOBAL PSEUDOBULK GUARD
    # ------------------------------------------------------------------
    n_samples_total = adata.obs[sample_key].nunique()
    pseudobulk_enabled = True

    if n_samples_total < 3:
        pseudobulk_enabled = False
        LOGGER.warning(
            "Dataset has %d samples total (<3). "
            "Pseudobulk DE is DISABLED globally; "
            "falling back to cell-level ranksum only.",
            n_samples_total,
        )

    # ------------------------------------------------------------------
    # Cell-level markers (always run)
    # ------------------------------------------------------------------
    markers_spec = CellLevelMarkerSpec(
        ...
        max_cells_per_group=int(
            getattr(cfg, "markers_downsample_max_per_group", 2000)
        ),
    ...
    # ------------------------------------------------------------------
    # Pseudobulk DE: cluster vs rest (GUARDED)
    # ------------------------------------------------------------------
    marker_genes_all_clusters = None

    if pseudobulk_enabled:
        layer_candidates = list(
            getattr(cfg, "counts_layers", ["counts_cb", "counts_raw"])
        )
        counts_layer = None
        ...
        if counts_layer is None and not bool(getattr(cfg, "allow_X_counts", True)):
            raise RuntimeError(
                "markers_and_de: none of counts_layers found in adata.layers "
                f"{layer_candidates}, and allow_X_counts=False"
            )
        ...
        pb_opts = PseudobulkDEOptions(
            min_cells_per_sample_group=int(
                getattr(cfg, "min_cells_target", 20)
            ),
            min_samples_per_level=int(
                getattr(cfg, "min_samples_per_level", 2)
            ),
        ...
        LOGGER.info(
            ...
            "(sample_key=%r, counts_layer=%r, "
            "min_cells_per_sample_group=%d, min_samples_per_level=%d)",
            pb_spec.sample_key,
    ...
    # ------------------------------------------------------------------
    # Condition DE (also GUARDED)
    # ------------------------------------------------------------------
    condition_key = getattr(cfg, "condition_key", None)
    conditional_de_genes_all_clusters = None

    if condition_key and pseudobulk_enabled:
        groups = pd.Index(
            pd.unique(adata.obs[groupby].astype(str))
        ).sort_values()

        cond_opts = PseudobulkDEOptions(
            min_cells_per_sample_group=int(
                getattr(cfg, "min_cells_condition", 20)
            ),
            min_samples_per_level=int(
                getattr(cfg, "min_samples_per_level", 2)
            ),
        ...
        condition_contrasts = (
            list(getattr(cfg, "condition_contrasts", [])) or None
        )
    ...
    adata.uns["markers_and_de"].update(
        {
            ...
            "pseudobulk_enabled": pseudobulk_enabled,
    ...
    if bool(getattr(cfg, "make_figures", True)):
        ...
        if pseudobulk_enabled:
            ...
            LOGGER.info(
                "Skipping pseudobulk plots (pseudobulk disabled globally)."
            )
    ...
    # ------------------------------------------------------------------
    out_zarr = output_dir / (
        str(getattr(cfg, "output_name", "adata.markers_and_de")) + ".zarr"
    )
    ...
    if bool(getattr(cfg, "save_h5ad", False)):
        out_h5ad = output_dir / (
            str(getattr(cfg, "output_name", "adata.markers_and_de")) + ".h5ad"
        )
        LOGGER.warning(
