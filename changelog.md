...
- added automatic per-sample lower-count filtering on `total_counts` using a combined lower MAD and lower quantile rule
...
- kept fixed `--min-counts` off by default, so the pipeline now applies conservative automatic lower-tail cleanup for ordinary datasets while leaving stronger intervention to explicit user choice
