...
Sample-level enrichment is currently CLI-only through `scomnom enrichment sample`; no sample-enrichment function is exported in these Python namespaces. See the [enrichment command reference](docs/markers-and-de/enrichment.md#sample-enrichment-pre-release) for its pre-release status and options.
